...
#define ECHO_OUT_BEFORE 19  //det var 36
...
LiquidCrystal_I2C lcdOut(0x26, 16, 2);  // second LCD for exit — check I2C address
int totalSpaces = 10;
...
bool carWasUnderOutAfter = false;
unsigned long gateOutOpenedAtMs = 0;
...
if (pollReader(readerIn, lastUidIn, lastSeenIn, uid)) {
    ...
    if (pollReader(readerOut, lastUidOut, lastSeenOut, uid)) {
        ...
        openExitGate();
        gateOutOpen = true;
        gateOutOpenedAtMs = millis();
        carWasUnderOutAfter = false;
    ...
    // 3. Kontrollera sensorer och uppdatera LCD (körs var 300 ms)
    if (now - lastPrintAtMs >= kPrintIntervalMs) {
        ...
        // --- Ingång: in_after — stäng grind efter att bilen passerat ---
        if (gateInOpen) {
            unsigned long elapsed = millis() - gateOpenedAtMs;
            if (elapsed > 3000) {
                ...
                Serial.printf("[IN_AFTER] elapsed=%lums dist=%.1f car=%s\n", elapsed, distAfter, carUnderSensor ? "yes" : "no");
            ...
            if (millis() - gateOpenedAtMs > 10000) {
                Serial.println("[IN_AFTER] timeout — closing gate");
                closeEntryGate();
                gateInOpen = false;
                carWasUnderInAfter = false;
            }
        ...
        // --- Utgång: out_before — visa meddelande på lcdOut ---
        float distOutBefore = outBeforeSensor.readDistanceCm(kSamplesPerReading);
        bool carDetectedOut = (distOutBefore != UltrasoundSensor::kInvalidDistanceCm && distOutBefore <= kCarPresentThresholdCm);
        Serial.printf("[OUT_BEFORE] dist=%.1f car=%s\n", distOutBefore, carDetectedOut ? "yes" : "no");

        if (carDetectedOut && !carAtExit) {
            carAtExit = true;
            lcdOut.clear();
            lcdOut.setCursor(0, 0); lcdOut.print("Goodbye!");
            lcdOut.setCursor(0, 1); lcdOut.print("Scan your card");
        } else if (!carDetectedOut && carAtExit) {
            carAtExit = false;
            lcdOut.clear();
            lcdOut.setCursor(0, 0); lcdOut.print("Exit");
            lcdOut.setCursor(0, 1); lcdOut.print("Drive safely!");
        }

        // --- Utgång: out_after — stäng grind efter att bilen passerat ---
        if (gateOutOpen) {
            unsigned long elapsedOut = millis() - gateOutOpenedAtMs;
            if (elapsedOut > 3000) {
                float distOutAfter = outAfterSensor.readDistanceCm(kSamplesPerReading);
                bool carUnderSensor = (distOutAfter != UltrasoundSensor::kInvalidDistanceCm && distOutAfter <= kCarPresentThresholdCm);
                Serial.printf("[OUT_AFTER] elapsed=%lums dist=%.1f car=%s\n", elapsedOut, distOutAfter, carUnderSensor ? "yes" : "no");
                if (carUnderSensor) carWasUnderOutAfter = true;
                if (carWasUnderOutAfter && !carUnderSensor) {
                    closeExitGate();
                    gateOutOpen = false;
                    carWasUnderOutAfter = false;
                }
            }
            if (millis() - gateOutOpenedAtMs > 10000) {
                Serial.println("[OUT_AFTER] timeout — closing gate");
                closeExitGate();
                gateOutOpen = false;
                carWasUnderOutAfter = false;
            }
